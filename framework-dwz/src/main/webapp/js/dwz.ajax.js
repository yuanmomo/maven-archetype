...
function dwzPageBreak(options){
	...
	if (op.rel) {
		...
		if (form) {
			$box.ajaxUrl({
				type:"POST", url:$(form).attr("action"), data: $(form).serializeArray(), callback:function(response){
					$box.find("[layoutH]").layoutH();
					if(op.callback !=undefined && op.callback !=null && op.callback != ""){
						var fun = new Function("response",op.callback);
						if ($.isFunction(fun)) {
							fun(response);
						}
					}
					$("div.pagination", $parent).each(function(){
						var $this = $(this); // 修改参数
						$this.attr("currentpage",op.data.pageNum);
						$this.attr("totalCount",response.returnValue.totalCount);
						$("#totalCount",navTab.getCurrentPanel()).text(response.returnValue.totalCount);
						$this.attr("numPerPage",response.returnValue.numPerPage);
						$this.pagination({
...
function navTabPageBreak(args, rel){
	// 取得回调函数
	var callBack = $($("div.pagination", navTab.getCurrentPanel())[0]).attr("callback");
	dwzPageBreak({targetType:"navTab", rel:rel, data:args,callback : callBack});
}
/**
 * 处理dialog中的分页和排序
 * 参数同 navTabPageBreak 
