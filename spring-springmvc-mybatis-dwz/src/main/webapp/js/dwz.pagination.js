		...
		pagination: function(opts){
			...
			return this.each(function(){
				...
				if (pc.hasPrev()){
					...
					_bindEvent($prev, pc.getCurrentPage()-1, pc.targetType(), pc.rel(),pc.getCallBack());
					_bindEvent($first, 1, pc.targetType(), pc.rel(),pc.getCallBack());
				} else {
				...
				if (pc.hasNext()) {
					...
					_bindEvent($next, pc.getCurrentPage()+1, pc.targetType(), pc.rel(),pc.getCallBack());
					_bindEvent($last, pc.numPages(), pc.targetType(), pc.rel(),pc.getCallBack());
				} else {
					$next.add($last).addClass("disabled").find(">a").hide();
				}
	
				$this.find(setting.nums$).each(function(i){
					_bindEvent($(this), i+interval.start, pc.targetType(), pc.rel(),pc.getCallBack());
				});
				$this.find(setting.jumpto$).each(function(){
					...
					$button.click(function(event){
						...
						if (pageNum && pageNum.isPositiveInteger()) {
							dwzPageBreak({targetType:pc.targetType(), rel:pc.rel(), data: {pageNum:pageNum},callback:pc.getCallBack()});
						}
					});
			...
			function _bindEvent($target, pageNum, targetType, rel,callback){
				$target.bind("click", {pageNum:pageNum}, function(event){
					dwzPageBreak({targetType:targetType, rel:rel, data:{pageNum:event.data.pageNum},callback:callback});
		...
		orderBy: function(options){
			...
			return this.each(function(){
				var $this = $(this).css({cursor:"pointer"}).click(function(){
					...
					dwzPageBreak({targetType:op.targetType, rel:op.rel, data:{orderField: orderField, orderDirection: orderDirection},callback:pc.getCallBack()});
				});
